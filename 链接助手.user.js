...
!(function () {
    ...
    // 网盘密码预处理
    if (/http:\/\/www\.38hack\.com\/\d+\.html/.test(location.href)) {
        // 爱之语
        let lines = document.querySelectorAll("div.down-line");
        if (lines.length)
            lines[lines.length - 1].appendChild(
                lines[0].previousElementSibling.previousElementSibling
            );
    } else if (/https:\/\/www\.mikuclub\.org\/\d+/.test(location.href)) {
        // 初音社
        let password = document.querySelector(".password1");
        if (password)
            document.querySelector("a.download").hash = password.value;
    } else if (/https:\/\/www\.acggw\.com\/\d+\.html/.test(location.href)) {
        // ACG港湾
        let ps = document.querySelectorAll(".single-content>p"),
            l = null,
            m = null;
        for (let p of ps) {
            if (p.innerText.startsWith("链接：")) l = p;
            if (l && p.innerText.startsWith("密码："))
                l.innerText += "#" + p.innerText.replace("密码：", "");
            if (m && p.innerText.startsWith("国外M盘："))
                m.innerText += "#" + p.innerText.replace("国外M盘：", "");
            if (p.innerText.startsWith("国外M盘：http")) m = p;
        }
    } else if (
        /http:\/\/www\.olecn\.com\/download\.php\?id=\d+/.test(location.href)
    ) {
        // 百度网盘资源
        ...
        for (var a of filetit.children) {
            if (a.href) {
                ...
                if (result)
                    a.href = result[1].startsWith("http")
                        ? result[1]
                        : "https://" + result[1];
            }
            if (
                a.innerHTML.startsWith("90网盘：") ||
                a.innerHTML.includes("90pan")
            ) {
    ...
    let YunDisk = {
        ...
        addCode2: function (a) {
            ...
            if (!regExp.test(a.hash.slice(1))) {
                ...
                let reg = new RegExp(
                    "\\s*(?:提[取示]|访问|查阅|密\\s*|艾|Extracted-code|key|password)[码碼]?\\s*[:： （(]?\\s*(" +
                        site.regStr +
                        ")|^\\s*[:：【\\[ （(]?\\s*(" +
                        site.regStr +
                        ")[】\\]]?$",
    ...
    if (YunDisk.sites[dealedHost]) YunDisk.autoFill(dealedHost);
        ...
        let RedirectPage = {
            ...
            wiki: function () {
                ...
                let a = document.querySelector(
                    'a.interlanguage-link-target[lang="zh"]'
                );
                let lastHref = null;
                if (!isZh) {
            ...
            mozilla: function () {
                ...
                let options = document.querySelectorAll(
                    "#select_language>option,#language-selector>option"
                );
                if (!isZh) {
                    if (jumpToZh) {
        ...
        // 跳转页面
        if (RedirectPage.sites[location.host])
            RedirectPage.redirect(location.host);
        // 维基百科及镜像页面
        else if (location.host.match(/.+wiki(?:\.sxisa|pedia)\.org/))
            RedirectPage["wiki"]();
        // 点击元素(文本或链接)
        else {
            // Mozilla开发者页面
            if (location.host == "developer.mozilla.org")
                RedirectPage["mozilla"]();

            // 页面监听
            document.addEventListener("mouseup", function (clickObj) {
                let e = clickObj.target ?? clickObj.originalTarget,
                    isTextToLink;
                if (e && !e.href) {
                    // 非链接
                    let flag = true,
                    ...
                    if (e.localName !== "a" && flag) {
                        let node =
                            selectNode || clickObj.explicitOriginalTarget;
                ...
                if (e && e.localName === "a") {
                    // 链接
                    let a = e;
                    if (location.href.includes("mod.3dmgame.com/mod/")) {
                        // mod.3dmgame.com
                    ...
                    if (isLinkText(a)) {
                        ...
                        if (url_regexp.test(a.innerText))
                            a.href = a.innerText.startsWith("http")
                                ? a.innerText
                                : "https://" + a.innerText;
                    }
                    // 含真实链接的跳转链接
            ...
            function textToLink(e) {
                if (
                    ...
                    !["www.google"].some((h) => location.host.includes(h))
                ) {
                    let span = null,
                        count = 0;
                    for (
                        let i = e.childNodes.length - 1, limit = 10;
                        i >= 0 && limit > 0;
                        i--
                    ) {
            ...
            function text2Link(node) {
                let result = url_regexp.exec(node.nodeValue),
                    span = null;
                if (result && !result[0].includes("@")) {
                    span = document.createElement("span");
                    span.className = "textToLink";
            ...
            function text2Links(node) {
                if (node.nodeValue.length < textLength) {
                    ...
                    let result = url_regexp_g.exec(node.nodeValue),
                        span = null,
                        count = 0;
                    if (result && !result[0].includes("@")) {
                        // console.log(result);
            ...
            let excludeSites = GM_getValue("excludeSites", [
                "v.qq.com",
                "v.youku.com",
                "blog.csdn.net",
                "cloud.tencent.com",
            ...
            // 注册菜单项该站链接保持默认打开方式
            if (!isDefault) {
                let menuID2 = GM_registerMenuCommand(
                    "该站链接保持默认打开方式",
                    function () {
            ...
            function add_blank(a) {
                if (isAddBlank && !isDefault) {
                    let result =
                        ...
                        ["prev", "next"].some(
                            (r) => r == a.attributes.rel?.nodeValue
                        ) ||
                        ["prev", "next", "nxt"].some((r) =>
                            a.className.includes(r)
                        ) ||
                        (/thread-\d+-\d+-\d+/.test(a.href)
